...
#ifdef __DOOLP_ALLOW_TCP_DEPRECATED
#include <doolp/doolp-doolpconnection-tcp.h>
#endif // #ifdef __DOOLP_ALLOW_TCP_DEPRECATED
#include <doolp/doolp-doolpconnection-xml.h>

Doolp::ConnectionTCPServer::ConnectionTCPServer ( Doolp::Forge * _forge, char * _type, 
...
void * Doolp::DoolpConnectionTCPServer_runAcceptThread ( void * arg )
{ return ((Doolp::ConnectionTCPServer*) arg)->runAcceptThread ( ); }

bool Doolp::ConnectionTCPServer::start()
{
  if ( forge == NULL )
...
void * Doolp::ConnectionTCPServer::runAcceptThread ( )
{
  ...
  while (true)
    {
      ...
      __DOOLP_Log ( "New Connection from %s:%d\n", 
		    ...
		    clientAddr.sin_port );
      if ( strcmp ( type, "TCP::XML" ) == 0 )
	{
	  ConnectionXML * newConnection = new ConnectionXML ( forge, clientSocket, -1, -1 );
	  ...
	  newConnection->status = ConnectionStatus_ToBeHandShaked;
	  newConnection->handle ();
	}
#ifdef __DOOLP_ALLOW_TCP_DEPRECATED
      else if ( strcmp ( type, "TCP::BIN" ) == 0 )
	{
	  ConnectionTCP * newConnection = new ConnectionTCP ( forge, clientSocket );
	  newConnection->name = inet_ntoa ( clientAddr.sin_addr );
	  __DOOLP_Log ( "Created connection at %p\n", newConnection );
	  Info ( "New %s Client from %s\n", type, newConnection->name );
	  newConnection->status = ConnectionStatus_ToBeHandShaked;
...
bool Doolp::ConnectionTCPServer::stop()
{
  return true;
}
